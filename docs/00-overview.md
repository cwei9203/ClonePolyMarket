...
| 编号 | 作为… | 我想… | 以便… |
|---|---|---|---|
| US-1 | 新用户 | 注册后立即获得 10,000 积分 | 无门槛开始体验 |
...
- 用户注册/登录（初始 10,000 积分）。
- 市场列表页（从 Polymarket 同步的活跃市场）。
- 市场详情页（当前赔率 + 历史走势图 + 下注面板）。
- 买入 Yes/No 份额（虚拟积分，锁定成交价）。
- 卖出持仓（按当前镜像价）。
- 自动结算与赔付（跟随 Polymarket 结算）。
- 个人持仓页 + 交易流水。
- 全站排行榜（净值榜 + 收益率榜）。
...
| **结算正确性** | 市场在 Polymarket 结算后，本站在下一同步周期内完成赔付，获胜方每份赔付 1 积分、失败方归零。 |
...
3. 结算完全跟随 Polymarket；本站不处理 Polymarket 判定为「50-50」等特殊结果时的边界（见开放问题 Q3）。
4. 首版为单体 Next.js 应用 + 托管 Postgres，不引入微服务。见 [ADR-001](./decisions/ADR-001-tech-stack.md)。
