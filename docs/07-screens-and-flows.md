...
│ 🥉 carol        14,100    +41.0% ↑        │
│ …                                          │
│ 87 你 (me)      12,300    +23.0%  ← 高亮   │
└──────────────────────────────────────────┘
```

- 前三名奖章；数字等宽右对齐（`tabular-nums`）。
- 当前用户所在行高亮，若不在当前页则底部/顶部固定「我的排名」条（来自 API `meta.me`）。
- 分页或无限滚动；指标切换即时重排（带过渡）。

### 3.4 我的持仓 `/portfolio`

**顶部概览卡**：净值、可用余额、持仓市值、总收益率（大字 + 涨跌色）。
...
时间倒序表：时间、市场、类型（买/卖/结算）、方向、份额、成交价、积分变动、交易后余额。用于复盘对账（对应 `trades` 表，[02 §3.4](./02-data-model.md#34-tradestrades交易流水只增不改)）。
...
| **破产**（余额 0） | 提示无法买入，引导卖出或（Post-MVP）领补给 |
